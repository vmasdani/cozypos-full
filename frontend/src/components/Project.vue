<template>
  ...
  <div class="flex m-2 items-center">
    <router-link to="/projects/new">
      <button class="bg-blue-500 text-white font-semibold p-2 rounded-lg">New</button>
    </router-link>
    <input class="ml-2 w-full border-2 border-gray-400 p-2 rounded-lg" placeholder="Search project.." />
  </div>
  <div class="m-2">
      ...
      >
        <router-link :to="`/projects/${project.project.id}`">
          <div>
            ...
            <div>Date: {{ makeReadableDateString(new Date(project.project.startDate)) }}</div>
            <div>Revenue: <span class="font-semibold">{{ formatIdr(project.income) }}</span></div>
            <div>Funding Cost: <span class="font-semibold">{{ formatIdr(project.totalManufacturingPrice) }}</span></div>
          </div>
        </router-link>
      </li>
    </ul>
