<template>
  ...
  >
    <div>
      <router-link
        :to="`/transactions/new?projectId=${state.selectedProject.id}`"
      >
  ...
  <div class="m-2" v-if="state.projectTransactionsView">
    ...
    <span class="font-bold"
      >{{ filteredProjectTransactionsView.length }}/{{
        state.projectTransactionsView.transactions.length
      }}</span
    >
    transactions
  </div>
  <div class="mt-3">
    <div class="mx-2">
      ...
      >
        <router-link
          :to="`/transactions/${transactionView.transaction.id}?projectId=${state.selectedProject.id}`"
        >
          <div
            class="shadow-lg border-2 border-solid border-blue-200 rounded-lg px-4 py-2"
          >
            <div class="flex justify-between items-center">
              <h3
                :class="`text-2xl font-bold ${comparePriceIsCustomColor(
                  transactionView
                )}`"
              >
                {{
                  formatIdr(
                    transactionView.transaction.priceIsCustom
              ...
              <div
                class="rounded-full shadow-lg text-white bg-gray-600 font-semibold px-2"
              >
                {{ transactionView.transaction.cashier }}
              </div>
            </div>
            <div class="text-sm font-bold text-gray-800 uppercase">
              Orig: {{ formatIdr(transactionView.totalPrice) }}
            </div>
            <div class="text-sm font-thin italic">
              {{
                transactionView.itemTransactions
                  .map(
                    (itemTransaction) =>
                      `${itemTransaction.item.name} x${itemTransaction.itemTransaction.qty}`
                  )
                  .join(", ")
              }}
            </div>
  ...
  setup() {
    ...
    const comparePriceIsCustomColor = (transactionView: TransactionView) => {
      if (!transactionView.transaction.priceIsCustom) {
        ...
        if (
          transactionView.totalPrice < transactionView.transaction.customPrice
        ) {
    ...
    const fetchProjectTransactionsView = async () => {
      try {
        state.requestStatus = "Loading";

        const response = await fetch(
          `${store.baseUrl}/projects/${state.selectedProject?.id ?? 0}/transactions`
        );
    ...
    const filteredProjectTransactionsView = computed(() =>
      state.projectTransactionsView
        ? state.projectTransactionsView.transactions.filter((transaction) => {
